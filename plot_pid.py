"""
Plot the PID feedback controller block diagram as a PNG.
"""
...
def circle(ax, x, y, r=0.28, label='+'):
...
# ── PID Controller box (split into 3 lanes)
box(ax, 2.6, 1.6, 2.0, 1.8, 'PID Controller', color='#2D6A9F')
...
# Σ at output of PID
circle(ax, 5.0, 2.5, r=0.25, label='Σ')

# ── Plant box
box(ax, 6.0, 1.9, 2.0, 1.2, 'Plant', sublabel='ẏ = −ay + bu', color='#2A7D4F')

# ── Output label
ax.text(11.5, 2.5, 'Output\ny(t)', ha='center', va='center', fontsize=10)

# ── Feedback path label (sensor)
box(ax, 4.5, 0.4, 2.0, 0.8, 'Sensor', color='#8B4513')

# ── Arrows
arrow(ax, 0.6, 2.5, 1.22, 2.5)          # setpoint → sum
arrow(ax, 1.78, 2.5, 2.6, 2.5)          # sum → PID
arrow(ax, 4.6, 2.5, 4.75, 2.5)          # PID → Σ (output sum)
arrow(ax, 5.25, 2.5, 6.0, 2.5)          # Σ → plant  (u)
arrow(ax, 8.0, 2.5, 11.2, 2.5)          # plant → output

# feedback line: output → bottom → sensor → sum
ax.plot([9.0, 9.0], [2.5, 0.8], color='black', lw=1.8, zorder=2)
ax.plot([9.0, 6.5], [0.8, 0.8], color='black', lw=1.8, zorder=2)
arrow(ax, 6.5, 0.8, 4.5, 0.8)           # → sensor box
arrow(ax, 4.5, 0.8, 1.5, 0.8)           # sensor → left
ax.plot([1.5, 1.5], [0.8, 2.22], color='black', lw=1.8, zorder=2)  # up to sum
...
plt.tight_layout()
plt.savefig('pid_diagram.png', dpi=150, bbox_inches='tight')
print("Saved pid_diagram.png")
